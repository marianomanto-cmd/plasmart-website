...
(function () {
  ...
  /* ---------- Estimador (modal) ----------
     El wizard vive en /cotizador/ y ademas se abre como modal desde los CTA.
     CSS y JS entran recien al abrirlo, para no sumarle peso al home. La URL
     pasa a /cotizador/ asi el estimado es linkeable, se puede mandar por
     WhatsApp y se mide como paso de embudo. El boton siempre es un <a> a
     /cotizador/: sin JS, navega igual. */
  (function () {
    ...
    function crear() {
      ...
      modal.className = 'modal cot-modal';
      modal.setAttribute('aria-hidden', 'true');
      modal.setAttribute('role', 'dialog');
      modal.setAttribute('aria-modal', 'true');
      modal.setAttribute('aria-label', 'Estimador de corte laser');
      modal.innerHTML =
        '<div class="modal-bg"></div>' +
        '<div class="modal-card cot-card">' +
          '<button class="modal-x" type="button" aria-label="Cerrar">\u2715</button>' +
          '<div id="cotizadorRoot"></div>' +
        '</div>';
      document.body.appendChild(modal);
      modal.querySelector('.modal-x').addEventListener('click', cerrar);
    ...
    function abrir(origen) {
      assets(function () {
        if (!modal) crear();
        window.PlasmartCotizador.mount(modal.querySelector('#cotizadorRoot'));
        modal.classList.add('open'); modal.setAttribute('aria-hidden', 'false');
        if (lenis) lenis.stop(); document.body.style.overflow = 'hidden';
        history.pushState({ cot: 1 }, '', '/cotizador/');
        window.dataLayer = window.dataLayer || [];
        window.dataLayer.push({ event: 'estimador_abrir', origen: origen || 'sitio' });
      });
    }

    /* Cerrar visualmente. Lo llama popstate, asi el boton "atras" del celular
       cierra el modal en vez de sacarte del sitio. */
    ...
    function cerrar() {
      if (history.state && history.state.cot) history.back(); // popstate hace el resto
      else cerrarVisual();
    }
    ...
    triggers.forEach(function (b) {
      b.addEventListener('click', function (e) {
        e.preventDefault();
        abrir(b.getAttribute('data-cotizador'));
      });
    });
